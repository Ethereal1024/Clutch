---
name: readme-doctor
description: Audit a repository's README against best-practice patterns and produce a prioritized punch list of fixes (P0/P1/P2), covering hero presence, install-to-first-success length, one-sentence clarity, audience-jargon match, scannability, and drift signals. Read-only diagnostic; edits only when the user explicitly asks.
---
...
Same three modes as other audit skills — Auto, Semi-auto (recommended), Manual. Use structured questions. Mode affects how many decisions are silent vs surfaced; it does not skip rubric checks.
...
3. If the user explicitly asks to open a PR: detect repository ownership, fork if needed, and open the PR with the punch list as the PR description
...
- Does not score visual quality of screenshots/GIFs (that is a visuals skill's job)
