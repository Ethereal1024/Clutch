...
def read_file(sandbox: Sandbox, config: Config, path: str, max_chars: int = 0) -> dict:
    limit = max_chars or config.read_max_chars
    try:
        p: Path = sandbox.resolve(path)
        if not p.is_file():
...
def write_file(sandbox: Sandbox, config: Config, path: str, content: str) -> dict:
    try:
        p: Path = sandbox.resolve(path)
...
def list_dir(sandbox: Sandbox, config: Config, path: str = ".") -> dict:
    try:
        p: Path = sandbox.resolve(path)
        if not p.is_dir():
