...
5. **隧道断开**：桥 503，agent 的 `.clc` 追加或工具调用会抛 `TransportError`——运行终止。
   ...
   恢复运行前 `derive_messages._repair_dangling` 会剥离不完整 tool_calls 块：只保留有实文的
   assistant（纯 thinking/空内容的悬空块整体丢弃），丢弃残缺 tool 消息与游离 tool——派生结果
   里任何 assistant 必带 content 或 tool_calls，续跑不再触发 OpenAI 400。
